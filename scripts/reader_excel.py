...
def _read_excel(filepath, timestamp, *, format_type, encoding, sheet_name=None) -> StatFileResult:
    ...
    warnings_list.append(
        "Excel: 不含变量标签/值标签等统计元数据，仅保留原始数据值和日期基准 | "
        "Excel: No variable labels/value labels metadata, only raw data & date origin preserved"
    )
    ...
    if sheet_name is not None:
        ...
        warnings_list.append(
            f"Excel 文件包含 {len(sheet_names)} 个工作表，已选择数据量最大的 '{selected_sheet}'（约 {sheet_sizes[selected_sheet]} 行）。"
